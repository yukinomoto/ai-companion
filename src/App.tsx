...
function App() {
  ...
  const { 
    ...
    playVoice, isMuted, setIsMuted, sessions 
    ...
    <div style={{ maxWidth: '600px', margin: '0 auto', height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif', border: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
      ...
      <div style={{ padding: '16px', backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0' }}>
        ...
        <span style={{ fontSize: '15px', fontWeight: 'bold', color: '#1e293b' }}>
          {messages.length > 0 ? 'チャット（相談モード）' : '新しい相談'}
        </span>
        <select 
          value={selectedVoice} 
          onChange={(e) => setSelectedVoice(e.target.value)}
          style={{ padding: '6px 12px', borderRadius: '20px', border: 'none', fontSize: '12px', backgroundColor: '#f1f5f9', color: '#475569', cursor: 'pointer', outline: 'none' }}
        >
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isAi ? 'flex-start' : 'flex-end', opacity: msg.isQuickResponse ? 0.7 : 1 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', flexDirection: isAi ? 'row' : 'row-reverse', maxWidth: '90%' }}>
                ...
                {/* AIのみアイコンを表示 */}
                {isAi && (
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '4px', fontSize: '18px' }}>
                    🤖
                  </div>
                )}
                ...
                <div style={{ 
                  ...
                  fontSize: '15px', 
                  whiteSpace: 'pre-wrap',
                  boxShadow: isAi ? '0 4px 12px rgba(0,0,0,0.05)' : 'none',
                  lineHeight: '1.5'
                }}>
                  {msg.text}
                ...
                {isAi && !msg.isQuickResponse && (
                  <button onClick={() => playVoice(msg.text)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '16px', padding: '8px 0', color: '#cbd5e1' }}>📢</button>
                )}
              </div>
            </div>
          );
